  ...
  <form method="post" action="/dashboard/players/{{ $player->slug }}" class="mb-5" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      ...
      <input type="number" class="form-control @error('height') is-invalid @enderror" id="height" name="height" required
        value="{{ old('height',$player->height) }}"><small class="text-muted">cm</small>
      @error('height')
      <div class="invalid-feedback">
        {{ $message }}
      </div>
    ...
    <div class="mb-3">
      ...
      <input type="number" class="form-control @error('weight') is-invalid @enderror" id="weight" name="weight" required
        value="{{ old('weight',$player->weight) }}"><small class="text-muted">kg</small>
      @error('weight')
      <div class="invalid-feedback">
        {{ $message }}
      </div>
    ...
    <div class="mb-3">
      ...
      <select class="form-select" name="position">
        <option value="{{ old('position',$player->position) }}" class="fw-bolder" selected>Striker</option>
